...
- (void)webViewDidFinishLoad:(UIWebView*)theWebView
{
    ...
    //Stan modified for loading local cordova
    NSString *path = [[NSBundle mainBundle] pathForResource:@"cordova-2.2.0" ofType:@"js" inDirectory:@"www"];
    NSString *jsCode = [NSString stringWithContentsOfFile:path encoding:NSUTF8StringEncoding error:nil];
    NSLog(@"data is %@",[theWebView stringByEvaluatingJavaScriptFromString:jsCode]);
    
    //Stan modified to add refresh header view
    if(_refreshHeaderView == nil){
        
        //Get scrollView
        webScroller = (UIScrollView *)[[webView subviews] objectAtIndex:0];
        [webScroller setDelegate:self];
        NSLog(@"scroller created");
        
        EGORefreshHeaderView *view = [[EGORefreshHeaderView alloc] initWithFrame:CGRectMake(0.0f, 0.0f - webScroller.bounds.size.height, self.view.frame.size.width, webScroller.bounds.size.height)];
    ...
    //Stan modified for pull down refresh
    _reloading = NO;
...
//Stan modified, do pull down to refresh
//-(void) refreshView:(UIRefreshControl*)refresh{
//    refresh.attributedTitle = [[NSAttributedString alloc] initWithAttributedString:@"Refreshing data..."];
//    
...
#pragma mark -
#pragma EGORefreshHeaderDelegate Methods

-(void) egoRefreshTableHeaderDidTriggerRefresh:(EGORefreshHeaderView *)view{
//    [webView reload];
//    [self loadView];
    [[self webView] reload];
...
-(BOOL) egoRefreshTableHeaderDataSourceIsLoading:(EGORefreshHeaderView *)view{
    return _reloading;
}

-(NSDate*) egoRefreshTableHeaderDataSourceLastUpdated:(EGORefreshHeaderView *)view{
    return [NSDate date];
}
